...
router.get('/:start', async (req, res) => {
  ...
  if (req.params.start >= 0) {
    ...
    FROM Users u,Notifications n WHERE u.IdUserOwner = n.IdUserOwner AND n.IdUserReceiver=? HAVING blacklist IS NULL ORDER BY n.DateCreation DESC LIMIT ?,30', [
    ...
    if (result.length > 0) {
      ...
      result.map((item, index) => {
        ...
        if (index + 1 === result.length) locals.sendResponse(res, 200, { data: result, IsRead: IsRead.length > 0 ? IsRead[0].IsRead : 0 }, true)
      })
