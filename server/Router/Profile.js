...
router.post('/EditInfoUser', auth, async function (req, res) {
  ...
  if ((UserName, Email, FirstName, LastName, FirstName, DataBirthday, Sexual, Gender, ListInterest) && Validate('Username', UserName) && Validate('Email', Email) && Validate('Name', FirstName) && Validate('Name', LastName) && isValidDate(DataBirthday) && getAge(DataBirthday) >= 18 && Biography && Biography.length <= 100 && (Sexual.trim() === 'Male' || Sexual.trim() === 'Female' || Sexual.trim() === 'Female Male' || Sexual.trim() === 'Male Female') && (Gender.trim() === 'Male' || Gender.trim() === 'Female') && JSON.parse(ListInterest).length && JSON.parse(ListInterest).length <= 5) {
    Sexual = Sexual.trim()
    Gender = Gender.trim()
...
router.post('/DeleteImage', auth, async function (req, res) {
  const locals = req.app.locals
  let ImagesUser = await locals.getImage(req.userInfo.Token, locals)
  console.log(JSON.parse(ImagesUser[0].Images)[req.body.index])
...
router.get('/GetUser/:userName', auth, async function (req, res) {
  ...
  if (req.params.userName) {
    ...
    if (IdUserReceiver) {
      ...
      if (ifNotBlock) {
        const result = await locals.select('Users', ['FirstName', 'UserName', 'Email', 'DataBirthday', 'LastName', 'City', 'Gender', 'Sexual', 'Biography', 'ListInterest', 'Images', 'LastLogin', 'Active'], {
        ...
        if (result[0]) {
          ...
          const values = {
            ...
            YourRating,
            CountRating,
            CountFriends: CountFriends[0].Count,
            CheckFriends: CheckFriends[0].Count,
          }
