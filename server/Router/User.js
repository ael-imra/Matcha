...
router.get('/active', function (req, res) {
  if (typeof req.query.token !== 'undefined' && req.query.token !== '') {
    fetch(`http://localhost:5000/user/verifierToken/${req.query.token}`).then(
      (data) => {
        if (data[0].Count === 1) {
          const sqlUpdate =
            'UPDATE `Users` SET `IsActive` = ?  WHERE `Token` = ?'
          const active = 1
          con.query(sqlUpdate, [active, req.query.token], (err, result) => {
            if (err) res.send('Error')
              ...
              con.query(
                sqlUpdate,
                [crypto.randomBytes(64).toString('hex'), req.query.token],
                (err, result) => {
                  if (err) res.send('Error')
                  else {
...
router.post('/ResetPassword', function (req, res) {
  if (
    ...
    fetch(`http://localhost:5000/user/verifierToken/${req.body.token}`).then(
      (data) => {
        if (data[0].Count === 1) {
          if (req.body.password === req.body.confirm) {
            ...
            con.query(
              sqlUpdate,
              [md5(req.body.password), req.body.token],
              (err, result) => {
                if (err) res.send('Error')
                else {
                  const sqlUpdate =
                  ...
                  con.query(
                    sqlUpdate,
                    [crypto.randomBytes(64).toString('hex'), req.body.token],
                    (err, result) => {
                      if (err) res.send('Error')
                      else {
                        res.send('account is active')
...
router.get('/verifierToken/:userToken', function (req, res) {
  if (
    typeof req.params.userToken !== 'undefined' &&
