...
router.get('/', async (req, res) => {
  ...
  const result = await locals.query(
    ...
    [req.userInfo.IdUserOwner, req.userInfo.IdUserOwner, req.userInfo.IdUserOwner, req.userInfo.IdUserOwner, req.userInfo.IdUserOwner]
  )
  if (result.length > 0) {
    const newObj = []
    result.map((item) =>
...
router.get('/:UserName/:index', async (req, res) => {
  ...
  if (req.params && req.params.UserName && req.userInfo.UserName !== req.params.UserName) {
    ...
    if (arr.length === 0 || arr.length < 24) arr.push('limit')
    arr.reverse()
