...
router.get('/',async (req,res)=>{
  ...
  {
    const newObj = []
    result.map(item=>newObj.push({IdUserOwner:item.IdUserOwner,messages:[{Content:item.Content,date:item.DateCreation.toISOString(),myMessage:item.MIdUserOwner === req.userInfo.IdUserOwner ? 1:0}],UserName:item.UserName,Images:JSON.parse(item.Images)[0],IsRead:item.IsRead}))
