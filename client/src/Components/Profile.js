...
export default function Profile(props) {
  ...
  const [userNameAndEmail, changeUserNameAndEmail] = React.useState({ email: '', userName: '', isProfileOfYou: '' });
      ...
      Axios.post(`/Profile/GetUser/${userName}`, {})
        .then(async (result) => {
          if (result.data !== 'User not found')
            changeInfoUser({ ...result.data, Images: JSON.parse(result.data.Images), ListInterest: JSON.parse(result.data.ListInterest), DataBirthday: result.data.DataBirthday.split('T')[0], Sexual: result.data.Sexual === 'Female Male' ? 'Male Female' : result.data.Sexual });
          let results = await Axios.post(`/Profile/CheckProfileOfYou/${userName}`, {});
          changeUserNameAndEmail({ email: result.data.Email, userName: result.data.UserName, isProfileOfYou: result.data !== 'User not found' ? results.data.isProfileOfYou : 'User not found', isNotReport: results.data.isNotReport });
        })
        .catch((error) => {});
    } catch (error) {}
