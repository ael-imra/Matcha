...
const FtInput = (props) => {
  ...
  let focus = (e) => {
    ...
    ctx.Mode === 'Light'
      ? (e.target.className = 'Input input-active-light')
      : (e.target.className = 'Input input-active-dark')
  }
  useEffect(() => {
    if (ArrError[props.index] === '')
      ArrError[props.index] = 'the input is empty'
  ...
  return (
    <input
      ...
      onFocus={focus}
      className={
        ctx.Mode === 'Light' ? 'Input input-light' : 'Input input-dark'
      }
      ref={(el) => (props.Ref.current[props.index] = el)}
    />
  )
}
