...
export default function PersonalInfo(props) {
    ...
    try {
      Axios.post('/Profile/EditInfoUser', props.InfoUser).then((result) => {
        ...
        localStorage.setItem(
          'userInfo',
          JSON.stringify({ ...userInfo, FirstName: props.InfoUser.FirstName, UserName: props.InfoUser.UserName, LastName: props.InfoUser.LastName })
        );
      });
    } catch (error) {}
  };
  ...
  let validationEdit = () => {
    ...
    inputsInfo.forEach(async (inputInfo, key) => {
      ...
      if (Await.length === 2) {
        if (props.InfoUser.ListInterest.length === 0) inputsError.push('ListInterest is not valid');
        if (inputsError.every((items) => items === '')) {
          UpdateInfoUser();
        } else props.ChangeError(inputsError);
  ...
  return (
    <div className='slide-in-right'>
      ...
      <div className='infoUser'>
        ...
        <input
          type='date'
          className='inputDateBirthday'
          style={{ color: ctx.Mode === 'Dark' ? 'white' : 'black' }}
          value={props.InfoUser.DataBirthday}
      ...
      <div className='infoUser'>
        ...
        <InputTest DefaultValue={props.InfoUser.City} />
      </div>
      <div className='infoUser'>
      ...
      <div className='infoUser'>
        ...
        <TextField
          ...
          select
          variant='outlined'
          value={props.InfoUser.Sexual}
          onChange={ChangeSexual}
          style={{ width: '100%', backgroundColor: 'var(--background-Input)', borderRadius: '8px' }}
          disabled={!props.UserNameAndEmail.isProfileOfYou}>
          {Sexual.map((option) => (
