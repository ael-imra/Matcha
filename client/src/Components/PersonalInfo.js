...
export default function PersonalInfo(props) {
  ...
  const UpdateInfoUser = () => {
    try {
      Axios.post('/Profile/EditInfoUser', props.InfoUser).then((result) => {
        ...
        props.changeUserNameAndEmail((oldValue) => ({
          ...oldValue,
        ...
        props.changeUser((oldValue) => ({
          ...oldValue,
          FirstName: props.InfoUser.FirstName,
          LastName: props.InfoUser.LastName,
          UserName: props.InfoUser.UserName,
        }))
  ...
  let validationEdit = () => {
    ...
    inputsInfo.forEach(async (inputInfo, key) => {
      if (key === 1) {
        const checkUserName = await Axios.post(`/Profile/UserReadyTake`, {
          UserName: inputInfo.value,
        })
      ...
      if (key === 2) {
        const checkEmail = await Axios.post(`/Profile/EmailReadyTake`, {
