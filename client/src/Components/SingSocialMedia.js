  ...
  const responseGoogle = (response) => {
    try {
      if (props.type === 'sing in') {
        ...
        changeUser({
          UserName: response.profileObj.name.replace(' ', '-').substring(0, 3) + response.accessToken.substring(0, 6),
    ...
    <div className='sing-social-media'>
      ...
      <Dialog
        ...
        <DialogActions>
          <button
            className='ft_btn'
            style={{ backgroundColor: 'rgb(49, 143, 181)', marginBottom: '10px', marginRight: '15px' }}
            onClick={async () => {
              let errorInput = [];
...
// 652872186498-sdfbthmnqp8tqnlnpmu3rsiv2v8rb8s5.apps.googleusercontent.com
// 2IU-NcWnJLWdLa-yauZGo7_N
